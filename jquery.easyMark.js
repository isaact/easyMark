...
/*
* Highlights the given term in all text nodes (transitive) for which 
this
* function is called. If lenient == true the pattern matching will be 
case- and
...
(function($) {
/*
* Map generated from ftp.unicode.org/Public/UNIDATA/UnicodeData.txt. 
Maps each
* upper-case basic alphabet character in the A-Z range to a regex 
character
...
//If lenient settings then we replace the search string with a regex 
character class A->[AaÄäå....]
if(settings.lenient) {
...
var inputPattern = new RegExp('\\b' + input.split(' ').join('|\\b'), 
'ig');
}
...
($(node).hasClass(settings.highlightClass)))) {
...
/*******************
* Creates a regexp for case- and accent-insensitive matching of a 
regular
* alphabetic string. For each character in the original string the 
output
* contains a regexp character class that matches the character itself 
and all
...
var cleanString = deaccentedString.replace(/([|()[{.+*?^$\\])/g, 
"\\$1");
...
/*******************
* Returns a string in which each accented and lower-case character from 
the
* input is replaced with the respective upper-case base character in the 
A-Z
...
result = result.replace(new RegExp(charToAccentedCharClassMap[key], 
"g"), key);
}
return result;
}
